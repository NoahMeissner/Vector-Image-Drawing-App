from Calculate_Bezier import Bresenham_Algorithm, bezier_curve
import numpy as np
...
class Draw:
    ...
    def draw_bezier(self, point1, point2, point3):
    ...
    def draw_polygon(self, vertices, polygon_color):
        ls = [(p.X(), p.Y()) for p in vertices]
        for index, point in enumerate(vertices):
            if point.get_bezier():
                pre = vertices[index - 1]
                control = point.get_control_Point()
                bezier = [pre.coordinates(), control.coordinates(), point.coordinates()]
                line = bezier_curve(bezier)
                ls.append(line)
        #points = [coord for vertex in ls for coord in vertex]
        polygon = Polygon(self.canvas)
        self.canvas = polygon.draw_polygon(ls)
        #self.canvas.create_polygon(points, fill=polygon_color)


    def check_polygon(self, point_list, polygon_color):
        all_polygons = []
        for index, point in enumerate(point_list):
            if point.get_polygon() is not None:
                ...
                for point in point_list:
                    if point.get_id() == start_point.get_id() or (start_track == True and point.get_id() != end_point.get_id()):
                        start_track = True
                        ls.append(point)
        ...
        for a in all_polygons:
            self.draw_polygon(a, polygon_color)



    def draw(self, points_list, line_color, polygon_color):
        self.check_polygon(points_list, polygon_color)
        if len(points_list) > 0:
            current_point = points_list[-1]
            if len(points_list) > 1:
                last_point = points_list[-2]
                print(current_point)
                self.draw_line(last_point.coordinates(), current_point.coordinates(), line_color)
    ...
    def draw_all(self, points_list, line_color, polygon_color):
        self.check_polygon(points_list,polygon_color)
        if self.draw_control_points:
        ...
        for i in range(1,len(list_p)):
            ...
            if len(list_p) > 0:
                if current_point.get_bezier():
                    self.draw_bezier(last_point.coordinates(),current_point.get_control_Point().coordinates(), current_point.coordinates())
                else:
                    self.draw_line(last_point.coordinates(), current_point.coordinates(), line_color)
