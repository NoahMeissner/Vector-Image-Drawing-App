...
"""
The Save class handles the process of saving a list of Point objects and associated properties to a JSON file.
It converts the list of Point objects into a JSON-compatible format and writes it to the specified file.
"""
...
class Save:
    def __init__(self, list_points, url, color_line, color_polygon):
        ...
        self.url = url
        json_scheme = create_file_json(self.create_json(), color_line, color_polygon)
    ...
    def create_json(self):
        ...
        for index, point in enumerate(self.list_points):
            ...
            if point.get_control_point() is not None:
                ...
                control_point = create_point_json(control.get_id(), control.get_x(), control.get_y(), None,
                                                  control.get_bezier(), control.get_polygon())

            if point.get_polygon() is not None:
                polygon = point.get_polygon()
                id_code = polygon.get_id()
            point_json = create_point_json(point.get_id(), point.get_x(), point.get_y(), control_point,
                                           point.get_bezier(), id_code)
